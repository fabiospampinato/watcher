...
- **Performant**: Native recursive watching is used when available (macOS and Windows and additionally Linux on Node 20.13/22.1+), and it's efficiently manually performed otherwise.
...
- `fs.watch`: Node's built-in `fs.watch` function is essentially garbage and you never want to use it directly.
  - Cons:
    - Recursive watching is only supported starting in Node 20 on Linux and broken until Node 20.13/22.1+.
...
- `native`: whether to use the native recursive watcher if available and needed.
  ...
  - the native recursive watcher is only available under macOS and Windows and if using Node 20.13/22.1+ then Linux as well.
...
- `recursive`: whether to watch recursively or not.
  ...
  - this is implemented natively by Node itself when `native` is `true`.
